...
from pymanifold.types import Bet, Group, JSONDict, LiteMarket, Market, User
from vcr import VCR
...
def validate_lite_market(market: LiteMarket) -> None:
    assert market.id
    assert hasattr(market, "creatorId")
    assert market.creatorUsername
    assert market.creatorName
    assert market.question
    assert market.mechanism
    assert market.outcomeType
    assert market.pool is None or isinstance(market.pool, (dict, int, float))
    assert market.volume >= 0
    assert market.volume24Hours >= 0
    assert market.uniqueBettorCount >= 0
    assert market.token in (None, "MANA", "CASH")
    assert all(
        hasattr(market, attr)
        for attr in [
            "creatorAvatarUrl",
...
def validate_market(market: Market) -> None:
    ...
    assert isinstance(market.description, (dict, str))
    assert isinstance(market.groupSlugs, list)
    assert isinstance(market.textDescription, str)
